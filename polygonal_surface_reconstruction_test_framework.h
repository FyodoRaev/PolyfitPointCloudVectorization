...
#include <CGAL/boost/graph/IO/OFF.h>   // запись OFF
#include <CGAL/boost/graph/IO/PLY.h>   // (по желанию) запись PLY

#include <typeinfo>                       // typeid()
#ifdef __GNUG__                           // деманглинг имён классов (GCC/Clang)
  #include <cxxabi.h>
  #include <memory>
#endif
...
int reconstruct (Reconstruction& algo, SMesh& model, const MIP_Solver&)
{
  ...
  std::cout << " Done. Time: " << t.time() << " sec. ";
        ...
        if (model.is_valid()) {
                std::cout << "\tReconstructed model has "
                          << model.number_of_faces() << " faces" << std::endl;

                // ---------- Сохраняем результат ----------


                const std::string out_file = "polyfit_result.off";

                if(CGAL::IO::write_OFF(out_file, model))           // <CGAL/boost/graph/IO/OFF.h>
                        std::cout << "\tMesh written to " << out_file << std::endl;
                else
...
// Estimate normals via PCA (if not present in PLY or noisy)
